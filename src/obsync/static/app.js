  ...
  $("#content").innerHTML = `<form class="settings-layout" id="settings-form">
    ...
    <section class="settings-card"><h3>${headingWithHelp("Local AI organization", "Optional local model settings used to title, summarize, categorize, tag, and link extracted content.")}</h3><p>Use Ollama, LM Studio, or another OpenAI-compatible local server. Obsync keeps syncing with deterministic rules if the model is offline.</p>
      <div class="field-grid">
        ...
        <div class="field">${labelWithHelp("llm-timeout", "Model timeout (seconds)", "Maximum time for each AI inference request. The default is 600 seconds; the quick connection check uses a separate 15-second limit.")}<input id="llm-timeout" type="number" min="5" max="3600" value="${escapeHtml(settings.llm_timeout_seconds || "600")}"><small>Default: 600 seconds per AI request. A sweep can make multiple requests.</small></div>
      </div>
  ...
  $("#content").innerHTML = `
    ...
    <form class="settings-layout" id="vault-settings-form">
      ...
      <section class="settings-card full-width sweep-overview"><div class="section-head"><div><span class="eyebrow">WHOLE-VAULT INTELLIGENCE</span><h3>Vault index</h3><p>Vault cache: <strong>${sweeps.indexed_notes}</strong> Markdown notes. Last successful Index Sweep: <strong>${sweeps.last_indexed_at ? relativeTime(sweeps.last_indexed_at) : "Never"}</strong>.</p></div><div class="settings-actions compact"><button class="secondary start-sweep" data-type="index" type="button">Start Index Sweep</button><button class="quiet start-sweep" data-type="index" data-full="true" type="button">Rebuild Whole Index</button></div></div>
        ...
        <p class="inline-status good">Index Sweep is always read-only. It records heading-aware chunks, canonical documents, folders, native tags, category hubs, existing links, and exact named-document references without editing notes.</p>
        ${sweepScheduleFields("vault-index", settings)}
      </section>
      <section class="settings-card full-width sweep-overview"><div class="section-head"><div><span class="eyebrow">ACTIVE VAULT CARE</span><h3>Maintenance Sweep</h3><p>Uses Local AI first to persist factual entities and exact source evidence, then to select links only from supported graph edges. It never appends an Obsync relationship section or links notes merely because they share words, folders, tags, or a document type.</p></div><div class="settings-actions compact"><button class="secondary start-sweep" data-type="maintenance" type="button">Start Maintenance Sweep</button></div></div>
      ...
      <section class="settings-card full-width"><h3>Whole-vault matching and links</h3><div class="field-grid compact-grid"><div class="field"><label for="existing-note-policy">Strong existing-note matches</label><select id="existing-note-policy"><option value="review">Send first adoption to Review</option><option value="auto">Adopt strong matches automatically</option></select><small>Exact source/content matches update automatically. Ambiguous matches always require review.</small></div><div class="field"><label for="vault-candidate-limit">Candidate notes per AI decision</label><input id="vault-candidate-limit" type="number" min="5" max="50" value="${escapeHtml(settings.vault_relationship_candidate_limit || "20")}"><small>Hybrid retrieval uses exact entities, evidence-backed graph paths, and vault structure.</small></div><div class="field"><label for="vault-relationship-confidence">Minimum AI relationship confidence</label><input id="vault-relationship-confidence" type="number" min="0.5" max="1" step="0.01" value="${escapeHtml(settings.vault_relationship_min_confidence || "0.72")}"><small>Every accepted link must copy a supported factual edge and exact source evidence.</small></div><div class="field"><label for="vault-link-limit">Maximum inline links per note</label><input id="vault-link-limit" type="number" min="1" max="20" value="${escapeHtml(settings.vault_link_limit || "3")}"><small>A conservative safety ceiling, not a target. Unsupported links are discarded.</small></div><div class="field"><label for="vault-timezone">Schedule timezone</label><input id="vault-timezone" value="${escapeHtml(settings.vault_schedule_timezone || "America/New_York")}"></div></div></section>
...
function sweepAiActivity(activity, type, activeSweep) {
  ...
  return {
    ...
    idle_message: isIndexOnly
        ? "This read-only sweep refreshes canonical documents, heading-aware chunks, folder hierarchy, tag vocabulary, category hubs, existing links, and exact references. Maintenance is the separate factual-analysis and editing process."
      : waitingForAi
  ...
  $("#content").innerHTML = `<div class="settings-layout"><div id="ai-live-host" class="ai-live-host" data-ai-live-host>${aiActivityMarkup(activity)}</div>
    ...
    <section class="settings-card"><h3>${headingWithHelp("Local AI connection", "Optional local classification through Ollama, LM Studio, or another OpenAI-compatible server.")}</h3>
      <div class="field-grid">
        ...
        <div class="field">${labelWithHelp("llm-timeout", "Model timeout (seconds)", "Maximum time for each AI inference request, including vault-model learning and each Maintenance Sweep decision.")}<input id="llm-timeout" type="number" min="5" max="3600" value="${escapeHtml(settings.llm_timeout_seconds || "600")}"><small>Default: 600 seconds per AI request. A sweep can make multiple requests.</small></div>
      </div>
