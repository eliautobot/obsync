...
- Indexes the complete Markdown vault and persists heading-aware chunks, canonical documents, exact mentions, factual edges, provenance, hierarchy, and entity specificity for whole-vault matching
...
- Learns each vault's organization and factual graph before link selection; every link must copy a precomputed typed edge with exact evidence instead of relying on a shared word or document type
- Rejects misleading date, number, metadata-label, and generic-word link anchors; the exact source sentence must support the proposed target
- Rebuilds tags and links from current Markdown before learning, flags exact duplicates, and keeps folder/index organization review-only
- Runs read-only Index Sweeps plus native Maintenance Sweeps with live model reasoning/output, progress, safe Stop, Review/automatic modes, operation-level diffs, and Undo Sweep
- Applies approved maintenance as ordinary inline `[[wikilinks]]` and YAML tags, with no visible Obsync maintenance block
...
python -m pip install --upgrade \
  "obsync-app @ git+https://github.com/eliautobot/obsync.git@v0.20.0"
obsync --version
obsync agent scan
```
...
Obsidian has no remote core API for this workflow. Obsync integrates through Obsidian's native vault formats. Its scheduled or manual whole-vault index records note content, heading-aware chunks, aliases, properties, tags, paths, folders, links, backlinks, stable identifiers, hashes, and modification times. Maintenance then extracts durable entities and directional factual claims into a private persistent Vault Knowledge Graph with exact quotes, offsets, content hashes, temporal state, and entity frequency. Hybrid retrieval shortlists graph targets and structurally relevant notes. Final link selection can only copy a supported edge and exact natural anchor; it cannot invent a new document relationship.

## Whole-vault sweeps and maintenance

Open **Obsidian Vault** to run or schedule two independent operations:

- **Index Sweep** is always read-only. It refreshes the whole-vault cache, deterministic document graph, heading-aware chunks, corpus profile, folder hierarchy, tag vocabulary, links, backlinks, and exact duplicate evidence without invoking Local AI or changing Markdown.
- **Maintenance Sweep** requires Local AI. It first extracts and persists grounded factual claims, then proposes links, tags, exact duplicate resolutions, and review-only organization. Every accepted link must copy an existing supported `source entity → typed predicate → target entity` edge and resolve it to one exact target document.

Both sweeps support **Start**, **Stop**, daily/weekly/monthly/custom schedules, live note-level progress, and no-overlap protection. Review mode is the safe default. Maintenance Review lets you apply individual operations from a card instead of accepting its links, tags, and organization as one bundle. Automatic mode carries a prominent warning because it can change existing entries without human approval. Every applied sweep change stores expected hashes, evidence, confidence, and complete before/after content. Concurrent user edits stop the affected recommendation, and **Undo Sweep** restores changes that are still current. Sweeps never automatically delete or merge notes.
