...
Replace `v0.13.1` with the desired release tag. A tag checkout is intentionally fixed to that release; check out `main` again before using `git pull` for later updates.
...
```bash
docker pull ghcr.io/eliautobot/obsync:0.13.1
```

Replace `0.13.1` with the desired version. Prefer a numbered tag for predictable deployments. If your Compose file references the published image, the complete update is:
