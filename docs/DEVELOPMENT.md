...
18. Run a read-only Index Sweep followed by Maintenance; verify factual extraction precedes link selection, every link copies a supported edge, live counters match Review, legacy tags survive cleanup, unchanged graph rows are reused, and model/provider traces remain bounded and stoppable.
19. Run the 511-note disposable release harness; require exact expected anchors, zero forbidden anchors, zero reciprocal noise, zero decision failures, no Review writes, SQLite integrity, selective apply, and byte-exact Undo.
