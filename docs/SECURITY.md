...
The Local AI page and Index/Maintenance Sweep panels can display provider-emitted reasoning and streamed output for the active and most recent inference. Authenticated same-origin server-sent events deliver those updates to the browser; each subscriber has a one-item coalescing queue that is removed when the stream disconnects. Browser sign-out and page-unload paths close their EventSource connection and pending auto-scroll timers. Per-session traces retain at most 80 bounded events in server memory and are not persisted as chat transcripts. Sweep traces are read-only and use the same cooperative Stop Sweep safety boundary as the underlying vault work. Reviewer feedback for an explicit re-review is stored with that document for auditability and is sent to the configured model endpoint for that run.
